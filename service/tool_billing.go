...
func ComputeToolCallQuota(usage ToolCallUsage, groupRatio float64) ToolCallResult {
	...
	addItem := func(toolName string, count int) {
		...
		quota := common.QuotaFromFloat(math.Round(totalPrice * common.QuotaPerUnit * groupRatio)) // [TRAXNODE] 回移上游 c9943d37a 计费加固
	...
	if usage.ImageGenerationCall {
		...
		quota := common.QuotaFromFloat(math.Round(price * common.QuotaPerUnit * groupRatio)) // [TRAXNODE] 回移上游 c9943d37a 计费加固
		items = append(items, ToolCallItem{
